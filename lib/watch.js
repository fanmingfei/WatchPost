...
exports.watchTree = function ( root, options, unwatchSuffix, unwatchPath, callback ) {
  if (!callback) {callback = options; options = {}}
  walk(root, options, function (err, files) {
    if (err) throw err;
    var fileWatcher = function (f) {
      var suffix = path.extname(f);
      var hasUnWatchSuffix = unwatchSuffix.indexOf(suffix);
      if (hasUnWatchSuffix > -1) {
        // console.log(f)
        return;
      }
      unwatchPath.forEach(function(x){
        var hasUnWatchPath = f.split(path.sep).indexOf(x);
        if (hasUnWatchPath > -1) {
          // console.log(f)
          return;
        }
      ...
      fs.watchFile(f, options, function (c, p) {
        ...
        // if (root === '/Users/fanmingfei/ldsnwork/server-tool/process.js') {
        // }
      })
